function data = lastraPatchTest2(data, DEBUG)
    ...
    fprintf('PATCH-TEST: impongo spostamenti e rotazioni al bordo\n');
    
    data.L = 100;
    data.poly= [0.01, 0.01, -0.02, 0.005, 0.003, -0.001];
    ...
    function addD(nds, wv, bx, by)
      ...
      for nd = nds(:)'
        ...
        y = data.coords(nd,2);
        % calcolo i valori
        valW  = a(1) + a(2)*x + a(3)*y + a(4)*x^2 + a(5)*x*y + a(6)*y^2;
        valBx = -(          a(2)   + 2*a(4)*x   + a(5)*y);   % dw/dx
        valBy = -(          a(3)   + a(5)*x     + 2*a(6)*y); % dw/dy
